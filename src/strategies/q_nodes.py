...
class QNodes(SIA):
    ...
    def aplicar_estrategia(
        ...
        print(
            f"\n   [QNodes Algorithm] 🧠 Inicializando análisis estructural de grafos..QNODES."
        )
    ...
    def funcion_submodular_k(self, deltas, omegas):
        ...
        if clave_delta not in self.memoria_delta:
            # MAX_MEMO = 10
            # if len(self.sia_subsistema.memo) >= MAX_MEMO:
            #     print("MEMOF1")
    ...
    def _bipartir_y_emd(
        ...
        idxs_alcance: list,
        dims_mecanismo: list,
        solo_emd: bool = False,  # ← nuevo parámetro
    ) -> tuple[float, np.ndarray]:
        """
        Versión cacheada de bipartir → distribucion_marginal → emd_efecto.
        solo_emd=True: cachea solo el escalar, no el vector dist (ahorra memoria).
        """
        if not hasattr(self, "_bipartir_emd_cache"):
            self._bipartir_emd_cache = {}

        clave = (tuple(sorted(idxs_alcance)), tuple(sorted(dims_mecanismo)))

        if solo_emd:
            # Caché ligero: solo guarda emd (escalar)
            clave_emd = ("emd_only", *clave)
            if clave_emd not in self._bipartir_emd_cache:
                ...
                self._bipartir_emd_cache[clave_emd] = emd
                # dist se descarta aquí → el tensor de bipartir queda libre
            return self._bipartir_emd_cache[clave_emd], None

        # Caché completo: guarda (emd, dist)
        if clave not in self._bipartir_emd_cache:
            ...
            dist = particion.distribucion_marginal()
            emd = emd_efecto(dist, self.sia_dists_marginales)
            self._bipartir_emd_cache[clave] = (emd, dist)
