...
class BruteForce(SIA):
    ...
    def aplicar_estrategia_k(
        ...
        idx = 0
        tiempo_ultimo_hito = time.time()

        for p_alcance, p_mecanismo in generador_particiones:
            idx += 1
            
            # 1. Print dinámico (Velocímetro): Se actualiza en la misma línea cada 500 iteraciones
            # para no destruir el rendimiento de la CPU con prints continuos.
            if idx % 500 == 0:
                print(f"   [Fuerza Bruta] Evaluados: {idx} candidatos... 🔍", end="\r")
            ...
            if emd_value < small_phi:
                # Calculamos delta de tiempo del hallazgo
                tiempo_actual = time.time()
                lapso = tiempo_actual - tiempo_ultimo_hito
                tiempo_ultimo_hito = tiempo_actual
                ...
                if emd_value == FLOAT_ZERO:
                    print(f"\n🛑 [Corte Perfecto] Se aborta la búsqueda: Φ es exactamente 0.0 en la iteración {idx}!")
                    break
        ...
        print(f"🏆 MIP Encontrada: {solucion_base.particion} (Φ = {solucion_base.perdida:.6f})")
        print(f"⏱️ Tiempo total de cómputo de la fila: {solucion_base.tiempo_ejecucion:.4f}s\n")
